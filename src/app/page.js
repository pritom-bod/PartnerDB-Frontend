...
export default function Page() {
  ...
  const sourceOptions = ["International", "National"];

  // Dynamic count calculations
  const getDynamicCount = (field, value) => {
    return rows.filter((row) => {
      if (field === "sector") return row[field] === value;
      if (field === "budget_amount") return row[field] === value;
      if (field === "consultant_source") {
        if (value === "National") return row["hq"]?.toLowerCase() === "bangladesh";
        if (value === "International") return row["hq"]?.toLowerCase() !== "bangladesh";
        return false;
      }
      return false;
    }).length;
  ...
  const selectAllSectors = () => setSelectedSectors(sectorOptions);
  ...
  const selectAllBudgets = () => setSelectedBudgets(budgetOptions);
  const selectNoneBudgets = () => setSelectedBudgets([]);
  ...
  const selectAllSources = () => setSelectedSources(sourceOptions);
  const selectNoneSources = () => setSelectedSources([]);
  ...
  const fetchRows = async ({
    ...
    try {
      ...
      selectedBudgets.forEach((b) => params.append("budget_amount", b));
      selectedSources.forEach((src) => params.append("consultant_source", src));
...
const capitalizeFirst = (str) => {
  ...
  useEffect(() => {
    ...
    fetchHqs();
  }, [currentPage, query, hq, selectedSectors, selectedBudgets, selectedSources]);
      ...
      <div className="flex gap-6">
        ...
        <div className="w-1/4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 space-y-6">
            {/* Sector Filter */}
            <div>
              <div className="bg-blue-200 text-gray-800 p-2 font-medium rounded-t-md">Sector</div>
              <div className="bg-white dark:bg-gray-700 p-4 border border-gray-200 dark:border-gray-600 rounded-b-md">
                <div className="flex gap-2 mb-2 text-blue-600 text-sm">
                  <span className="cursor-pointer hover:underline" onClick={selectAllSectors}>Select All</span> |
                  <span className="cursor-pointer hover:underline" onClick={selectNoneSectors}>Select None</span>
                </div>
                <div className="overflow-y-auto max-h-48 space-y-2 scrollbar-thin scrollbar-thumb-dark-red-700 scrollbar-track-gray-200">
                  {sectorOptions.map((opt) => (
                    <label key={opt} className="flex items-center text-gray-900 dark:text-white text-sm">
                      <input
                        type="checkbox"
                        value={opt}
                        checked={selectedSectors.includes(opt)}
                        onChange={handleSectorChange}
                        className="mr-2"
                      />
                      {opt} ({getDynamicCount("sector", opt)})
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div>
              <div className="bg-blue-200 text-gray-800 p-2 font-medium rounded-t-md">Budget Amount</div>
              <div className="bg-white dark:bg-gray-700 p-4 border border-gray-200 dark:border-gray-600 rounded-b-md">
                <div className="flex gap-2 mb-2 text-blue-600 text-sm">
                  <span className="cursor-pointer hover:underline" onClick={selectAllBudgets}>Select All</span> |
                  <span className="cursor-pointer hover:underline" onClick={selectNoneBudgets}>Select None</span>
                </div>
                <div className="overflow-y-auto max-h-48 space-y-2 scrollbar-thin scrollbar-thumb-dark-red-700 scrollbar-track-gray-200">
                  {budgetOptions.map((opt) => (
                    <label key={opt} className="flex items-center text-gray-900 dark:text-white text-sm">
                      <input
                        type="checkbox"
                        value={opt}
                        checked={selectedBudgets.includes(opt)}
                        onChange={handleBudgetChange}
                        className="mr-2"
                      />
                      {opt} ({getDynamicCount("budget_amount", opt)})
                    </label>
                  ))}
                </div>
              </div>
            ...
            {/* Consultant Source Filter */}
            <div>
              <div className="bg-blue-200 text-gray-800 p-2 font-medium rounded-t-md">Consultant Source</div>
              <div className="bg-white dark:bg-gray-700 p-4 border border-gray-200 dark:border-gray-600 rounded-b-md">
                <div className="flex gap-2 mb-2 text-blue-600 text-sm">
                  <span className="cursor-pointer hover:underline" onClick={selectAllSources}>Select All</span> |
                  <span className="cursor-pointer hover:underline" onClick={selectNoneSources}>Select None</span>
                </div>
                  ...
                  {sourceOptions.map((opt) => (
                    <label key={opt} className="flex items-center text-gray-900 dark:text-white text-sm">
                      <input
                        ...
                        className="mr-2"
                      />
                      {opt} ({getDynamicCount("consultant_source", opt)})
                    </label>
                  ))}
                </div>
              </div>
